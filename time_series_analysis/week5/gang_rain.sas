...
filename rain "/home/u64002670/time_series/강릉강수량.csv" encoding="euc-kr";
/* CSV 파일 불러오기 */
...
proc export data=series dbms=csv outfile= "/home/u64002670/time_series/rain_k.csv"; run;
proc import datafile="/home/u64002670/time_series/rain_k.csv" out=work.series dbms=csv replace; getnames = yes; run;


proc sgplot data=work.series;
    title "강릉 월별 강수량(원자료)";
    series x=date y=precip;
    xaxis label="날짜 (1911.10-2025.09)";
    yaxis label="강수량 (mm)";
run;
...
/* 1st */
proc arima data=series;
   identify var = precip(1, 12) nlag=24 noprint; 
   estimate p=(12) q=(1)(12 24) method=ml plot;
...
proc arima data=series;
   identify var = precip(1, 12) nlag=24 noprint; 
   estimate p=(12) q=(1)(12 24) method=ml plot noint;
   forecast lead=12 out=fore; 
run; quit;
...
proc arima data=series;
   identify var=precip(1,12) nlag=24 noprint;
   estimate q=(1)(12 24) method=ml plot;
   forecast lead=12 out=fore;
run; quit;
...
proc arima data=series;
   identify var=precip(1,12) nlag=24 noprint;
   estimate q=(1)(12 24) method=ml noint plot;
   forecast lead=12 out=fore;
run; quit;



/*arima pdqPDQ_s : arima (0,1,1)(0,1,2)_12*/
...
data _null_;
   ...
   if vtype(date)='C' then do;
      ...
      else d = input(cats(strip(date),'-01'), anydtdte.);              /* 'YYYY-MM-01' 등 유연 파싱 */
   end;
   else do; /* 숫자형 */
...
proc sgplot data=fore_sep(where=(year(date)>=2023));
   ...
   /* 만약 PI를 미래만 그리고 싶으면 l95/u95 대신 l95_future/u95_future를 위처럼 만들어 쓰세요. */
...
/*****  | 그림4.28 |   국내 외국인 입국자 수 자료의  모형 적합 후의 잔차 시계열그림 ****/
/*
...
/*******│그림 4.29│ - │그림 4.31│ 국내 외국인 입국자 수 자료의  모형  적합 잔차의 SACF, SPACF, 포트맨토검정 SAS 결과 ******/
/*
proc arima data=res plots(unpack)=series(corr) ;
   identify var=residual nlag=24 ;
run ;quit;

/*****  │표 4.│ 국내 외국인 입국자 수 자료의  모형을 이용한 12시점 이후 예측값   ****/
/*
data fore; 
	set fore; 
	time=_n_; 
	if lvisit ne . then forecast=.; 
    visit= exp(lvisit);
    visit1 = exp(forecast);
run;
...
/*****  │그림 4.32│ 국내 외국인 입국자 수 자료의  모형을 이용한 예측 시계열도   ****/
/*
proc gplot;
   plot visit*time=1 visit1*time=2/ frame legend overlay haxis=0 to 144 by 24; 
run; quit;
*/